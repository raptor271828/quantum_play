...
if simulating:
    domain = schro.rectangular_1_particle_domain((20,20),11)

    V = np.log(((domain.X**2 + domain.Y**2)**1 ))*0.35
    V[V==-inf] = V[V != -inf].min()
    V -= 0.5
    initial_psi = schro.normalized_2d_gaussian(domain.X,domain.Y,0.5, dX=8) #* np.exp(1j*domain.Y*3.5)
    ...
    np.save(out_path+file_prefix+"_domain_whr.npy",domain_params)
    np.save(out_path+file_prefix+"_psi.npy", psi)
    np.save(out_path+file_prefix+"_t.npy", t)
else:
    energy_shift = -0.5

    domain_params = np.load(out_path+file_prefix+"_domain_whr.npy")
